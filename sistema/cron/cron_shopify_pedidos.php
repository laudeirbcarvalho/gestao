        ...
        foreach ($orders['orders'] as $order) {
            ...
            if ($result->num_rows == 0) {
                $sql = "INSERT INTO pedidos_shopify (n_pedido, order_id, customer_id, total_price, financial_status, created_at, operador, data_import)
                        VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
                $stmt = $conn->prepare($sql);
                $stmt->bind_param("ssssssss", $n_pedido, $order_id, $customer_id, $total_price, $financial_status, $created_at, $operador, date('Y-m-d H:i:s'));
                $stmt->execute();
                ...
                foreach ($order['line_items'] as $item) {
                    ...
                    $stmt_item->bind_param("iisiisds", $order_id, $item_id, $produto_nome, $quantidade, $preco_unitario, $total_item, $variant_id, $sku);
                    $stmt_item->execute();
                }
...
file_put_contents(PATH .'sistema/cron/cron_log.txt', date('Y-m-d H:i:s') . " - Script Pedidos ShopiFy executado\n", FILE_APPEND);
//echo PATH .'sistema/cron/cron_log.txt';
?>