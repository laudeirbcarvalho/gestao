...
// Consulta o total de pedidos no ano atual
$sql_total = "SELECT 
...
$total_pedidos_ano = $row_total['total_pedidos_ano'];
...
LEFT JOIN clientes_shopify c ON p.customer_id = c.id_shopify
WHERE w.status = 'integrado' 
    AND YEAR(w.data_hora) = YEAR(CURDATE()) -- Filtra pelo ano atual
    AND (p.financial_status = 'paid' OR p.financial_status = 'paid_shopify')
    AND c.central_id = '$sessao_central'
...
if ($result->num_rows > 0) {
    ...
    echo "<p>Total de Pedidos no Ano: " . $total_pedidos_ano . "</p>"; // Exibe o total de pedidos no ano
